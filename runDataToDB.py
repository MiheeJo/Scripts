...
class DBService(object):
  ...
  def createDB(self):
    ...
    runs = sqlalchemy.Table('rpc_noisy_strips', metadata,
    sqlalchemy.Column('id', sqlalchemy.Integer, sqlalchemy.Sequence('rpc_seq', optional=True),  primary_key = True),
    sqlalchemy.Column('runnumber', sqlalchemy.Integer),
  ...
  def deleteFromDB(self, runNumber=None):
    metadata = sqlalchemy.MetaData()
    table = sqlalchemy.Table('rpc_noisy_strips', metadata, 
    sqlalchemy.Column('id', sqlalchemy.Integer, sqlalchemy.Sequence('rpc_seq', optional=True),  primary_key = True),
  ...
  def insertToDB(self, dataList):
    ...
    for data in dataList:
      ins = table.insert().values(
          runnumber=data['run'], exec_date=data['date'], roll=data['rollName'],
          dead_strips=data['deadStrips'], masked_strips=data['maskedStrips'],
          strips_to_mask=data['stripsToMask'], strips_to_unmask=data['stripsToUnmask'],
          rate_hz=data['noise'], rate_hz_cm2=data['noisyStrips']
          )
...
class FileReader(object):
  ...
  def __contentCheck(self, contents):
    ...
    for run in contents[1:]:
      if not re.match(self.__runDataPattern, run, re.VERBOSE):
        if not re.match(self.__runDataPatternNan, run, re.VERBOSE):
          contentMeta['correct'] = False
  ...
  def read(self, fileName):
    ...
    for run in contents[1:]:
      ...
      runData['rollName'] = data[0]
      runData['deadStrips'] = data[1]
      runData['maskedStrips'] = data[2]
      runData['stripsToMask'] = data[3]
      runData['stripsToUnmask'] = data[4]
      runData['noise'] = data[5]
      runData['noisyStrips'] = data[6]
